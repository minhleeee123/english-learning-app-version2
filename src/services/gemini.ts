...
export const generateReadingSession = async (
  ...
  try {
    const response = await ai.models.generateContent({
      model: modelId,
      contents: `Write an engaging reading passage (approx 300-500 words) at CEFR level ${level}.
      
      Structure the text into multiple small, distinct paragraphs for better readability.
      ...
      After the story, provide 10 multiple-choice reading comprehension questions based on the text.`,
      config: {
        responseMimeType: "application/json",
        responseSchema: readingSchema,
        systemInstruction: "You are an English teacher creating reading materials. The story should be coherent, natural, and structured with clear paragraph breaks.",
      },
    });
